      ...
      <div class="col-md-12">
        <div class="box box-primary">
          <div class="box-header with-border">
            <h3 class="box-title">Nueva Oferta de Trabajo</h3>
          </div>
          <!-- /.box-header -->
          <!-- form start -->
...
export default {
  ...
  mounted() {
    this.fetchLoginToken().then(() => {
      let data = [this.getCategories(), this.getCurrencies(), this.getStates()];
  ...
  methods: {
    ...mapActions(["fetchLoginToken","getCurrencies", "getStates", "getCategories","postOffer"]),
    submit()
    {
      if(!this.$v.$invalid)
      {
        ...
        this.postOffer({token: access_token, data})
        .then(() => {
          this.$router.push({path:"/offers",params:{"message": "Agregado correctamente."}})
        })
        .catch(() => {
