...
export default {
  computed: {
    ...mapState({
      ...
      link: state => state.applicationLink
    }),
    access_token() {
  ...
  methods: {
    ...mapActions(["fetchLoginToken", "getApplication", "getApplicationLink"]),
    loadData(id) {
      let params = { token: this.access_token, id };
      this.getApplication(params).then(() => {
        this.getApplicationLink(params);
      });
    }
  }
};
